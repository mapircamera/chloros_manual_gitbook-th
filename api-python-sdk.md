...
### ติดตั้งผ่าน pip**การติดตั้งมาตรฐาน:**```bash
pip install chloros-sdk
```**พร้อมรองรับการติดตามความคืบหน้า:**```bash
pip install chloros-sdk[progress]
```**การพัฒนาการติดตั้ง:**```bash
...
1. เปิด**Chloros หรือ Chloros (เบราว์เซอร์)**และเข้าสู่ระบบผู้ใช้<img src=".gitbook/assets/icon_user.JPG" alt="" data-size="line">แท็บ หรือเปิด**CLI**2. ป้อนข้อมูลรับรอง Chloros+ ของคุณแล้วเข้าสู่ระบบ
...
```**พารามิเตอร์:**| พารามิเตอร์                 | พิมพ์ | ค่าเริ่มต้น                   | คำอธิบาย                           |
| - | - | - | - |
...
| `backend_startup_timeout` | ภายใน  | `60`                      | หมดเวลาสำหรับการเริ่มต้นแบ็กเอนด์ (วินาที) |**ตัวอย่าง:**```python
...
สร้างโปรเจ็กต์คลอรอสใหม่**พารามิเตอร์:**| พารามิเตอร์      | พิมพ์ | ที่จำเป็น | คำอธิบาย                                              |
| - | - | - | - |
| `project_name` | STR  | ใช่      | ชื่อโครงการ                                     |
| `camera`       | STR  | No       | เทมเพลตกล้อง (เช่น "Survey3N\_RGN", "Survey3W\_OCN") |**การส่งคืน:**`dict` - Project creation response**ตัวอย่าง:**```python
# Basic project
...
นำเข้ารูปภาพจากโฟลเดอร์**พารามิเตอร์:**| พารามิเตอร์     | พิมพ์     | ที่จำเป็น | คำอธิบาย                        |
| - | - | - | - |
| `folder_path` | str/เส้นทาง | ใช่      | เส้นทางไปยังโฟลเดอร์ที่มีรูปภาพ         |
| `recursive`   | บูล     | No       | ค้นหาโฟลเดอร์ย่อย (ค่าเริ่มต้น: เท็จ) |**การส่งคืน:**`dict` - Import results with file count**ตัวอย่าง:**```python
# Import from folder
chloros.import_images("C:\\DroneImages\\Flight001")
...
กำหนดการตั้งค่าการประมวลผล**พารามิเตอร์:**| พารามิเตอร์                 | พิมพ์ | ค่าเริ่มต้น                 | คำอธิบาย                     |
| - | - | - | - |
...
**ดัชนีที่มีอยู่:**NDVI, NDRE, GNDVI, OSAVI, CIG, EVI, SAVI, MSAVI, MTVI2 และอื่นๆ**ตัวอย่าง:**```python
# Basic configuration
chloros.configure(
    vignette_correction=True,
    reflectance_calibration=True,
...
ประมวลผลภาพโครงการ**พารามิเตอร์:**| พารามิเตอร์           | พิมพ์     | ค่าเริ่มต้น      | คำอธิบาย                               |
| - | - | - | - |
...
รับการกำหนดค่าโครงการปัจจุบัน**การส่งคืน:**`dict` - Current project configuration**ตัวอย่าง:**```python
...
รับข้อมูลสถานะแบ็กเอนด์**การส่งคืน:**`dict` - Backend status**ตัวอย่าง:**```python
status = chloros.get_status()
...
ปิดระบบแบ็กเอนด์ (หากเริ่มต้นโดย SDK)**ตัวอย่าง:**```python
...
ฟังก์ชั่นอำนวยความสะดวกบรรทัดเดียวในการประมวลผลโฟลเดอร์**พารามิเตอร์:**| พารามิเตอร์                 | พิมพ์     | ค่าเริ่มต้น         | คำอธิบาย                    |
| - | - | - | - |
...
| `progress_callback`       | เรียกได้ | `None`          | ความคืบหน้าการโทรกลับ              |**การส่งคืน:**`dict` - Processing results**ตัวอย่าง:**```python
from chloros_sdk import process_folder
...
### แบ็กเอนด์ไม่เริ่มต้น**ปัญหา:**SDK fails to start backend**โซลูชั่น:**1. ตรวจสอบว่ามีการติดตั้ง Chloros Desktop แล้ว:
...
### ตรวจไม่พบใบอนุญาต**ปัญหา:**SDK warns about missing license**โซลูชั่น:**1. เปิด Chloros, Chloros (เบราว์เซอร์) หรือ Chloros CLI แล้วเข้าสู่ระบบ
...
### ข้อผิดพลาดในการนำเข้า**ปัญหา:**`ModuleNotFoundError: No module named 'chloros_sdk'`**โซลูชั่น:**```bash
# Verify installation
...
### หมดเวลาการประมวลผล**ปัญหา:**Processing times out**โซลูชั่น:**1. เพิ่มการหมดเวลา:
...
### พอร์ตมีการใช้งานแล้ว**ปัญหา:**Backend port 5000 occupied**โซลูชั่น:**```python
# Use different port
...
3.**ปิดการใช้งานดัชนีที่ไม่จำเป็น**```python
...
### ถาม: ฉันสามารถใช้ SDK บนเซิร์ฟเวอร์ที่ไม่มี GUI ได้หรือไม่**A:** Yes! Requirements:
...
### ถาม: ฉันสามารถเผยแพร่แอปที่สร้างด้วย SDK ได้หรือไม่**A:** SDK code can be integrated into your applications, but:
...
### ถาม: SDK รองรับ async/await หรือไม่**A:**Current version is synchronous. For async behavior, use `wait=False` or run in separate thread:
...
ตัวอย่างทั้งหมดที่แสดงไว้ที่นี่ได้รับการทดสอบและพร้อมสำหรับการผลิต คัดลอกและปรับใช้ให้เหมาะกับกรณีการใช้งานของคุณ***## ใบอนุญาต**ซอฟต์แวร์ที่เป็นกรรมสิทธิ์** - ลิขสิทธิ์ (c) 2025 MAPIR Inc.
